...
function waitForVideo(url, timeoutMs){
  return new Promise((resolve, reject)=>{
    ...
    const done = ()=>{ clearTimeout(t); v.removeAttribute("src"); v.load(); resolve(url); };
...
function runVideo(prompt, stillUrl){
  if(!state.apiKey) throw new Error("Video needs a Pollinations API key (Settings). Still generated instead.");
  return { url: videoUrl(prompt, stillUrl), type: "video/mp4", stream: true };
}
function hydrateVideos(root){
  if(!root) return;
  const nodes = [...root.querySelectorAll("video[data-src]")];
  if(!hydrateVideos.io){
    hydrateVideos.io = new IntersectionObserver((entries)=>{
      entries.forEach(en=>{
        const v = en.target;
  ...
  nodes.forEach(v=>hydrateVideos.io.observe(v));
}
async function generate(){
  ...
  const job = {
    id, status:"queued", prompt: raw || "locked identity", fullPrompt: prompt,
    ...
    created: Date.now(), cost
  };
  state.jobs.push(job); state.credits -= cost; state.busy = true;
...
renderGallery = function(el, jobs){
  ...
  el.innerHTML = jobs.map(j=>{
    ...
    if(j.status==="animating"){
      const poster = j.poster || j.remote || "";
      const clip = (j.kind||"").startsWith("video") ? j.src : "";
      const media = clip
        ? `<video data-src="${clip}" poster="${poster}" muted loop playsinline preload="auto"></video>`
        : `<img src="${j.src}" alt="" loading="eager">`;
      return `<article class="card video-frame"><span class="lbl">BUFFERING</span>${media}<div class="cap">Still ready · clip streaming in…</div></article>`;
    }
    ...
    const media = (j.kind||"").startsWith("video")
      ? `<video data-src="${j.src}" poster="${j.poster||j.remote||""}" muted loop playsinline preload="none"></video>`
...
function openLb(id){
  ...
  if(vid){
    ...
    if(isVid){
      vid.poster = j.poster || j.remote || "";
      vid.preload = "auto";
      if(vid.src !== j.src) vid.src = j.src;
      vid.play().catch(()=>{});
    }else{
      vid.pause(); vid.removeAttribute("src"); vid.load(); img.src = j.src;
    }
  }else img.src = j.src;
